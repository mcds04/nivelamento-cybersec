# 🛡️ Nivelamento em Cybersecurity - <p align="center">
  <img src="https://docker.com" alt="Docker Logo" width="120"/>
</p>


Neste projeto, realizei a configuração completa do meu ambiente de desenvolvimento local, estruturado especificamente para estudos e práticas de segurança da informação (Cybersec).

---

## 🛠️ Tecnologias e Ferramentas Utilizadas

* **VS Code** – Editor de código principal com extensões personalizadas.
* **Git & GitHub** – Controle de versão e hospedagem do repositório para o portfólio.
* **WSL2 (Ubuntu)** – Subsistema Windows para Linux, permitindo rodar um ambiente nativo Linux (Ubuntu) diretamente no Windows 11 para execução de ferramentas de segurança.

---

## 🚀 Práticas Realizadas neste Laboratório

1. [x] Instalação e provisionamento do **WSL2 com Ubuntu**.
2. [x] Recuperação e gerenciamento de credenciais via terminal Linux (Root).
3. [x] Integração do **Git Bash** como terminal integrado padrão no VS Code.
4. [x] Vinculação de chaves e identificação global do Git.
5. [x] Criação da documentação base do projeto.

---

## 📂 Como testar o ambiente

Se você deseja replicar essa estrutura, certifique-se de ter o WSL2 ativo em sua máquina Windows e o Git devidamente configurado:

```bash
# Verificar se o WSL está ativo e qual a distribuição instalada
wsl -l -v

# Validar as configurações globais do Git
git config --list
```
